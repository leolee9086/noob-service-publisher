...
window.event_bridge = eventBridge;
if(!(await workspaceHandler.exists(path.join('data','storage','berry','noob-service-publisher','favicon.png')))){
const content = await workspaceHandler.readFile(path.join('data','servicies','noob-service-publisher','favicon.png'))
console.log(content)
await window.saveData('favicon.png',content)

}
window.saveData('config',JSON.stringify(设置))
eventBridge.on("ready", () => {
  console.log("ready");
});
eventBridge.handler("设置首页", (data) => {
  eventBridge.invoke("noobSyPublisherMain", "设置首页成功", { id: data.id });
});
...
app.use("/favicon/*", (req, res) => {
  res.sendFile(设置.网站图标);
});
app.use(
  "/fonts/*",
  express.static(path.join(appDir, "stage", "build", "fonts"))
);
...
app.use(
  "/appearance",
  express.static(path.join(workspaceDir, "conf", "appearance"))
);
        ...
        async (req, res) => {
          ...
          if (!req.query.clear) {
            if (fs.existsSync(cachePath)) {
              let { updated } = await 核心api.getBlockAttrs({ id: id }, "");
              let state = fs.statSync(cachePath);
              if (state) {
                let cacheTime = require("dayjs")(state.mtime).format(
                  "YYYYMMDDHHmmss"
                );
          ...
          if (!cached) {
            ...
            if (!fs.existsSync(cachePath)) {
              pathConstructor.initFilep(cachePath, content, function (err) {
                if (err) {
                  throw err;
                }
              });
...
api.describeApi("/getPrivateBlock", {
  名称: "获取私有块内容",
...
async function cacheAll() {
    ...
    data.forEach((doc) => {
      fetch(`http://127.0.0.1/block?id=${doc.id}&&clear=true`).then(
        async () => {
          ...
          event_bridge.call("noobMain", "status__msg", {
            msg: `${doc.id}发布缓存刷新`,
          });
        }
      );
  ...
  document
    .querySelector("#preViewer")
    .setAttribute("src", `http://127.0.0.1/block?id=${设置.首页}&&clear=true`);
}
